...
class ReplayBuffer(TorchDataset):
    ...
    def add(self, sample: Dict[str, Any], sample_filenames: Dict[str, Any],
        ...
        num_features = image_features.shape[1]
        self.faiss_index_e = faiss.IndexFlatL2(num_features)
        original_features = image_features.copy()
        self.faiss_index_e.add(original_features)
        ...
        if self.maximize_diversity:
            ...
            if self.faiss_index.ntotal == 0:
                ...
                euclidean_similarity, _ = self.faiss_index_e.search(original_features, 1)
                cos_similarity = self.faiss_index.search(image_features, 1)[0][0][0]
                similarity = 0.5 * cos_similarity + 0.5 *(1 / (1 + np.sqrt(euclidean_similarity)))
