...
from datasets import Kitti, Robotcar, Euroc
...
class Slam:
    def __init__(self, config):
        ...
        elif self.online_dataset_type == 'Euroc':
            self.online_dataset = Euroc(
                self.online_dataset_path,
                "MH_05_difficult",
                config.dataset.frame_ids,
                config.dataset.scales,
        ...
        if self.do_adaptation and config.depth_pose.batch_size > 1: # and False:
            ...
            replay_buffer_path_s = config.replay_buffer.load_path_s
            replay_buffer_path_s.mkdir(parents=True, exist_ok=True)
            replay_buffer_state_path_s = replay_buffer_path_s / 'buffer_state_s.pkl'
            replay_buffer_state_path_s = replay_buffer_state_path_s if \
                replay_buffer_state_path_s.exists() else None
            self.replay_buffer_s = ReplayBuffer(
                replay_buffer_path_s,
                self.online_dataset_type,
                replay_buffer_state_path_s,
                self.online_dataset.height,
                self.online_dataset.width,          
                self.online_dataset.scales,
                self.online_dataset.frame_ids,
                ...
                batch_size=config.depth_pose.batch_size-1,
                maximize_diversity=config.replay_buffer.maximize_diversity,
                max_buffer_size=config.replay_buffer.max_buffer_size*4,
                similarity_threshold=config.replay_buffer.similarity_threshold,
                similarity_sampling=config.replay_buffer.similarity_sampling,
            )
        else:
            self.replay_buffer = None
            self.replay_buffer_s = None
    ...
    def step(self):
        ...
        with torch.no_grad():
            ...
            # online_features = self.predictor.models['depth_encoder'](online_image)[2].detach()
            online_features = online_features.mean(-1).mean(-1).cpu().numpy()

        if self.replay_buffer is not None:
            sim = self.replay_buffer.add(online_data,
                                   self.online_dataset.get_item_filenames(self.current_step - 1),
                                   online_features,
                                   verbose=True)
            sim1 = self.replay_buffer_s.add(online_data,
                                   self.online_dataset.get_item_filenames(self.current_step - 1),
                                   online_features,
                                   verbose=True)
        if self.replay_buffer is not None:
            ...
            training_data = online_data   
        if self.replay_buffer_s is not None:
            replay_data_s = self.replay_buffer_s.get(online_data, online_features)
            if replay_data_s:
                training_data_s = self._cat_dict(online_data, replay_data_s)
            else:
                training_data_s = online_data
        else:
            training_data_s = online_data
        # =================================================
            ...
            if self.do_adaptation:
                if sim > 0.75:
                    outputs, losses = self.predictor.adapt(online_data,
                                                        training_data,
                                                        steps=self.adaptation_epochs, sim = sim, prue = True)

                else:
                    outputs, losses = self.predictor.adapt(online_data, training_data_s, steps=self.adaptation_epochs, sim = sim, prue = False)
        else:
            outputs, losses = self.predictor.adapt(online_data, None, sim = sim, prue = False)
        ...
        if self.logging:
            ...
            if PLOTTING and (not self.current_step % 50 or optimized):
                self.plot_metrics()
    ...
    def _pose_graph_to_2d_trajectory(pose_graph):
        ...
        trajectory = np.asarray([[p[0, 3], p[1,3], p[2, 3]] for p in poses])
        return trajectory
    ...
    def plot_trajectory(self):
        ...
        ax.plot3D(pred_trajectory[:, 0], pred_trajectory[:, 1], '--.', label='pred')
        ax.plot3D(gt_trajectory[:, 0], gt_trajectory[:, 1], '--.', label='gt')
        plt.axis('equal')
        plt.legend()
