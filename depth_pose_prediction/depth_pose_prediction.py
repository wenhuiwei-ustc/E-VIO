...
class DepthPosePrediction:
    def __init__(self, dataset_config: DatasetConfig, config: Config, use_online: bool = False):
        ...
        # Construct networks ==============================
        self.models = {}
        self.models['depth_encoder'] = SECT(model='SECT', drop_path_rate=0.3)
        self.models['depth_decoder'] = DepthDecoder(
            self.models['depth_encoder'].num_ch_enc, self.scales)
        self.models['pose_encoder'] = PoseEncoder(model='SECT', drop_path_rate=0.3)
        ...
        self.online_models = {}
        if self.use_online:
            self.online_models['depth_encoder'] = SECT(model='SECT', drop_path_rate=0.3)
            self.online_models['depth_decoder'] = DepthDecoder(
                self.online_models['depth_encoder'].num_ch_encoder, self.scales)
            self.online_models['pose_encoder'] = PoseEncoder(model='SECT', drop_path_rate=0.3)
        ...
        for model_name, m in self.models.items():
            if m is not None:
                m.to(self.device)
                if model_name == 'depth_encoder' or model_name == 'depth_decoder':
                    self.parameters_to_train += list(m.parameters())
                else:
                    if model_name == 'pose_encoder':
                        for param in self.models[model_name].stem.parameters():
                            param.requires_grad = False
                        for param in self.models[model_name].stages.parameters():
                            param.requires_grad = False
                    else:
                        for param in self.models['pose_decoder'].net.parameters():
                            param.requires_grad = False
                    self.parameters_to_train_pose += list(m.parameters())
        ...
        # Set up optimizer ================================
        self.optimizer = optim.AdamW(self.parameters_to_train, self.learning_rate)
        self.optimizer_pose = optim.AdamW(self.parameters_to_train_pose, self.learning_rate)
        self.lr_scheduler = optim.lr_scheduler.StepLR(self.optimizer, self.scheduler_step_size, 0.1)
        self.lr_scheduler_pose = optim.lr_scheduler.StepLR(self.optimizer_pose, self.scheduler_step_size, 0.1)
        self.epoch = 0
        if use_online:
            self.online_optimizer = optim.AdamW(self.online_parameters_to_train, self.learning_rate)
        else:
    ...
    def adapt(self,
              ...
              online_index: int = 0,
              steps: int = 1,
              online_loss_weight: Optional[float] = None, sim = 1, prue = True):
        ...
        # if sim < 0.5:
        self._set_adapt(sim, freeze_encoder=True)
        # prune_init(args, self.models['pose_encoder'])  
    ...
    def predict_from_images(
            ...
            relative_angle: Optional[Tensor] = None,
    ):
        ...
        if return_loss:
            ...
            inputs = {
                ...
                ('relative_angle', 0): relative_angle.to(self.device)
            }
    ...
    def save_model(self) -> None:
        ...
        optimizer_pose_save_path = save_folder / 'pose_optimizer.pth'
        to_save = {
        ...
        to_save = {
            'optimizer': self.optimizer_pose.state_dict(),
            'scheduler': self.lr_scheduler_pose.state_dict()
        }
        torch.save(to_save, optimizer_pose_save_path)
    ...
    def _set_adapt(self, sim, freeze_encoder: bool = True) -> None:
        ...
        for model_name, model in self.models.items():
            ...
            for name, param in model.named_parameters():
                ...
                # if 'pose_decdoer' in model_name:
                #         param.requires_grad = False
                # if 'dpeth_encdoer' in model_name:
                #         param.requires_grad = False
                ...
                if sim > 0.75:
                    ...
                    if 'dpeth_encdoer' in model_name:
                        param.requires_grad = False
                    # if 'stem.1.conv' in 'pose_encoder':
                    #     param.requires_grad = False
    ...
    def _compute_velocity_loss(
        ...
        rotation_loss = torch.zeros(batch_size, device=self.device).squeeze()
        num_frames = 0
        for frame in self.frame_ids:
            ...
            if frame == 0:
                ...
                pred_rotation = outputs[('axis_angle', 0, -1)]
            else:  # frame == 1
                pred_translation = outputs[('translation', 0, 1)]
                pred_rotation = outputs[('axis_angle', 0, -1)]
            ...
            gt_rotation = torch.linalg.norm(inputs[('relative_angle', frame)], dim=-1).squeeze()
            pred_rotation = torch.linalg.norm(pred_rotation, dim=-1).squeeze()

            dot_product = torch.sum(pred_rotation * gt_rotation, dim=-1)
            cosine_similarity = dot_product / (pred_rotation * gt_rotation + 1e-8)
            angle_diff = torch.acos(torch.clamp(cosine_similarity, -1.0, 1.0))
            
            # gt_rotation_conjugate = gt_rotation * torch.tensor([1, -1, -1], device=gt_rotation.device)
            # relative_quaternion = pred_rotation * gt_rotation_conjugate
            # angle_diff = 2 * torch.acos(torch.clamp(relative_quaternion[..., 0], -1.0, 1.0))

            rotation_loss += F.l1_loss(angle_diff, torch.zeros_like(angle_diff),
                                       reduction='none')  # separated by sample in batch
            num_frames += 1
        velocity_loss /= num_frames
        rotation_loss /= num_frames
        velocity_loss = 2.0 * velocity_loss + 1.5 * rotation_loss
