...
class Robotcar(Dataset):
    def __init__(
        ...
        self.velocity, self.angle = self._load_velocity()
        self.relative_distances, self.relative_angles = self._compute_relative_distance()
    ...
    def _load_velocity(self) -> List[float]:
        ...
        for sequence in self.sequences:
            ...
            column_list_r = ['roll', 'pitch', 'yaw']
            data_r = pd.read_csv(ins_file, usecols=column_list_r)
            data_r = data_r.to_numpy()
            interpolated_roll = interp1d(raw_timestamps, data_r[:, 0])(self.timestamps)
            interpolated_pitch = interp1d(raw_timestamps, data_r[:, 1])(self.timestamps)
            interpolated_yaw = interp1d(raw_timestamps, data_r[:, 2])(self.timestamps)
            theta = np.vstack((interpolated_roll, interpolated_pitch, interpolated_yaw)).T
    ...
    def _load_global_poses(self) -> np.ndarray:
        ...
        for sequence in self.sequences:
            ...
            rtk_file = self.data_path / sequence / 'gps' / 'ins.csv'
            ...
            utm = utm.dot(np.array([[0, 0, -1], [0, 1, 0], [1, 0, 0]]))
            utm = utm * -1
    ...
    def _compute_relative_distance(self) -> List[float]:
        ...
        for index in range(1, len(self.timestamps)):
            relative_distance, relative_angle = self._load_relative_distance(index)
            relative_distances.append(relative_distance)
            relative_angles.append(relative_angle)
        return relative_distances, relative_angles
    ...
    def __getitem__(self, index: int) -> Dict[Any, Tensor]:
        ...
        for frame_id in self.frame_ids[1:]:
            ...
            item['relative_angle', frame_id] = self.relative_angles[index + frame_id]
...
def undistort_images(data_path_in: str, models_path: str) -> None:
    ...
    data_path_in = data_path_in.rstrip('/') + '_distorted2'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_path', type=str, default='../../Robotcar')
    parser.add_argument('--models_path', type=str, default='../../Robotcar')
