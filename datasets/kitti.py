...
class Kitti(Dataset):
    def __init__(
        ...
        # print(self.left_depth_filenames)

        # Filter data to meet minimum distance requirements
        self.relative_distances, self.relative_angs = self._compute_relative_distance()
    ...
    def _compute_relative_distance(self) -> List[float]:
        ...
        for index in range(1, len(self.timestamps)):
            relative_distance, relative_ang = self._load_relative_distance(index)
            relative_distances.append(relative_distance)
            relative_angs.append(relative_ang)
        return relative_distances, relative_angs
    ...
    def _filter_by_index(self, keep_indices: Union[np.ndarray, List[int]]) -> None:
        ...
        keep_indices = keep_indices[:len(keep_indices)-8]  ## 04: -8, 05: -1, 06: -3, 7: -0, 09: -3, 10: -1
        ...
        # print(len(self.left_depth_filenames))
    ...
    def _filter_by_distance(self, min_distance: float) -> None:
        ...
        # print(len(self.relative_distance))
    ...
    def __getitem__(self, index: int) -> Dict[Any, Tensor]:
        ...
        for frame_id in self.frame_ids:
            # print(img_filenames[index + frame_id])
        ...
        for frame_id in self.frame_ids[1:]:
            ...
            item['relative_angle', frame_id] = self.relative_angs[index + frame_id] 
    ...
    def _load_relative_distance(self, index: int) -> float:
        ...
        cur_ang = np.loadtxt(str(self.velocity_filenames[index]))[20:23]
        delta_theta = cur_ang * delta_timestamp
...
if __name__ == '__main__':
    ...
    parser.add_argument('--odom_path', default='..\\')
